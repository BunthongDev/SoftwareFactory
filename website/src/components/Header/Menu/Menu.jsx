...
// 1. Receive the pre-fetched 'data' as a prop
const Menu = ({ data }) => {
  const pathname = usePathname();
  const [fixedHeader, setFixedHeader] = useState(false);
  const [openMenuMobile, setOpenMenuMobile] = useState(false);

  // 2. Destructure settings and menu_items from the data prop
  const { settings = {}, menu_items = [] } = data;

  // This useEffect is for a UI interaction (scroll), which is perfectly fine.
  useEffect(() => {
      ...
      >
        <div className="container flex items-center justify-between h-20">
          <Link className="menu-left-block" href="/">
            <Image
              src={settings.logo_url || "/images/AnajakSoftware-page-not-found.png"}
              width={480}
              height={200}
              alt="logo"
              priority={true}
          ...
          <div className="menu-center-block h-full">
            <ul className="menu-nav flex items-center gap-1 h-full">
              {menu_items.map((item) => (
                <li key={item.label} className="nav-item h-full flex flex-col items-center justify-center relative">
                  <Link
                    className={`nav-link text-title flex items-center gap-1 px-1 py-2 transition-colors duration-200 font-bold ${isActive(item.link) ? "text-blue-600 font-bold" : "text-gray-700 hover:text-blue-600"}`}
                    href={item.link}
                  >
                    <span>{item.label}</span>
                  </Link>
                  {isActive(item.link) && <motion.div className="absolute bottom-4 h-1.5 w-1.5 bg-blue-600 rounded-full" layoutId="active-dot" />}
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="menu-mobile-main">
            <div className="container">
              <ul className="menu-nav-mobile h-full pt-1 pb-1">
                {menu_items.map((item) => (
                  <li key={item.label} className={`nav-item-mobile h-full flex-column gap-2 pt-2 pl-3 pr-3 pb-2 pointer rounded-lg ${isActive(item.link) ? "bg-blue-100" : ""}`}>
                    <Link
                      className="nav-link-mobile flex items-center justify-between"
                      href={item.link}
                      onClick={() => setOpenMenuMobile(false)}
                    >
                      <span className={`font-bold ${isActive(item.link) ? "text-blue-600" : ""}`}>{item.label}</span>
                    </Link>
                  </li>
                ))}
              </ul>
